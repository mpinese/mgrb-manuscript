...
hc = hail.HailContext(log = 'log/99_acmg.log', tmp_dir = 'tmp/hail')

#vds = hc.read('../MGRB.phase2.SNPtier12.match.vqsr.minrep.locusAnnot.vds')

#acmg_kt = hail.KeyTable.import_bed('ACMG_refGene.bed')

#tier_1_sample_list = [s.strip() for s in open('../tier1.sample_list', 'rt')]
#tier_2_sample_list = [s.strip() for s in open('../tier2.sample_list', 'rt')]

#vds = (vds
#    .filter_variants_table(acmg_kt)
#    .filter_samples_list(tier_1_sample_list + tier_2_sample_list)
#    .repartition(112)
