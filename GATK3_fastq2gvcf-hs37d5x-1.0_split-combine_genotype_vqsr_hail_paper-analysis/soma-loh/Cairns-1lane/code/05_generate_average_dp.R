...
sample_ids = read.table("../04_good_samples.txt", header = FALSE, stringsAsFactors = FALSE)[,1]
sample_files = list.files("..", "\\.afs\\.bgz$", full.names = TRUE)
message(sprintf("Found %d sample AF files.", length(sample_files)))
names(sample_files) = gsub("\\..*", "", gsub(".*\\/", "", sample_files))
sample_files = sample_files[names(sample_files) %in% sample_ids]
message(sprintf("%d sample files remain after restricting to good samples", length(sample_files)))
...
saveRDS(result, "../05_dp_stats.rds")
write.table(result, file = "../05_dp_stats.tsv", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
