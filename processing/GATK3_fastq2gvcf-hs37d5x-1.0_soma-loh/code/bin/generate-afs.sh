...
# Temporary files
loci_intervals=$(mktemp --suffix=.intervals)
genotype_vcf=$(mktemp --suffix=.vcf)

# Convert the TSV-format loci file into a GATK format .intervals
xz -dc "${LOCI_TSVXZ}" | awk '(NR>1) {print $1 ":" $2 "-" $2}' > "${loci_intervals}"
...
java -Djava.io.tmpdir="${TMPDIR}" -Xmx2G -jar "${GATK}" -T HaplotypeCaller -R "${REF_FA}" -L "${loci_intervals}" -ip 100 -I "${INPUT_BAM}" -o "${genotype_vcf}"
...
BEGIN {
    FS="\t"
    OFS="\t"
    print "chrom", "pos", "dp", "ad"
}

(($4 == "G" || $4 == "C" || $4 == "A" || $4 == "T") && ($5 == "G" || $5 == "C" || $5 == "A" || $5 == "T")) {
    split($9, fields, ":")
    split($10, values, ":")
    for (i in fields)
