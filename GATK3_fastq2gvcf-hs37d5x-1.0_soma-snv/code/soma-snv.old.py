...
class Interval:
    def __init__(self, seq=None, start0=None, end1=None):
        assert start0 < end1
        self.seq = seq
        self.start0 = start0
        self.end1 = end1
    ...
    # Enables transparent sorting of class instances
    # and simplifies intersection logic.
    def __lt__(self, other):
        """
        >>> a = Interval('1', 100, 200)
        >>> b = Interval('10', 10, 20)
        >>> c = Interval('10', 10, 30)
        >>> d = Interval('10', 11, 12)
        >>> a < a
        ...
        if self.seq == other.seq:
            if self.start0 == other.start0:
                return self.end1 < other.end1
            return self.start0 < other.start0
    ...
    def __eq__(self, other):
        ...
        False
        """
        return self.seq == other.seq and self.start0 == other.start0 and self.end1 == other.end1


    def __repr__(self):
        return 'Interval({}, {}, {})'.format(self.seq, self.start0, self.end1)


    def __str__(self):
        return '{}:[{},{})'.format(self.seq, self.start0, self.end1)


    def intersects(self, other):
        """
        Other is of type Interval
        Returns True if any overlap, else False
        Touching intervals are not considered to overlap.

        >>> a = Interval('1', 100, 200)
        >>> b = Interval('1', 100, 150)
        >>> c = Interval('1', 50, 250)
        >>> d = Interval('1', 50, 100)
        >>> e = Interval('1', 120, 130)
        >>> f = Interval('2', 120, 130)
        >>> a.intersects(b)
        True
        >>> b.intersects(a)
        True
        ...
        if self.seq != other.seq:
            return False
        if self < other:
            return self.end1 > other.start0
        else:
            return other.end1 > self.start0
...
class Blacklist:
    ...
    def load_regionlist(self, regionlist):
        temp_regions = {}
        for region in regionlist:
            chrom, _, interval = region.partition(':')
            if interval.find('-') == -1:
                start1 = interval
                end1 = interval
            else:
                start1, end1 = interval.split('-')
            if chrom not in temp_regions:
                temp_regions[chrom] = []
            temp_regions[chrom].append(Interval(chrom, int(start1)-1, int(end1)))

        for chrom in temp_regions.keys():    
            temp_regions[chrom].sort()
    ...
    def load_bed(self, instream):
        ...
        for line in instream:
            ...
            if chrom not in temp_regions:
                temp_regions[chrom] = []
            temp_regions[chrom].append(Interval(chrom, int(start0), int(end1)))
    ...
    def intersects(self, query):
        """
        >>> bl = Blacklist()
        >>> bl.load_regionlist(['1:100-150', '1:200-250', '2:300-350', '2:400-450', '4:100-120'])
        >>> bl.intersects(Interval('2', 0, 50))
        False
        >>> bl.intersects(Interval('2', 100, 150))
        False
        >>> bl.intersects(Interval('2', 250, 320))
        True
        >>> bl.intersects(Interval('2', 330, 340))
        True
        >>> bl.intersects(Interval('2', 345, 360))
        True
        >>> bl.intersects(Interval('2', 420, 500))
        True
        >>> bl.intersects(Interval('2', 520, 600))
        False
        >>> bl.intersects(Interval('3', 100, 200))
        False
        >>> bl.intersects(Interval('4', 50, 150))
        True
        >>> bl.intersects(Interval('5', 50, 150))
        False
        """
        if query.seq not in self.regions:
            return False

        if query.seq != self.last_chrom:
            self.last_index = 0

        intersection = False
        for self.last_index in range(self.last_index, len(self.regions[query.seq])):
            if self.regions[query.seq][self.last_index].intersects(query):
                intersection = True
                break

            if query < self.regions[query.seq][self.last_index]:
                intersection = False
                break
...
#        self._precompute_dense_thresholds()
    ...
    def process_mpileup_stream(self, instream, outstream_vars, outstream_sens):
        ...
        for line in instream:
            ...
            temp = time.clock()
            blacklist_intersection = self.blacklist.intersects(Interval(fields.chrom, fields.pos - 1, fields.pos))
